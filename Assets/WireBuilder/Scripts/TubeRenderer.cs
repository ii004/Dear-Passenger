...
public class TubeRenderer : MonoBehaviour
{
	...
	private Vector3[] CalculateCircle(int index)
	{
		...
		var aspectRatio = _radiusTwo / _radiusOne;

		for (int i = 0; i < _sides; i++)
		{
			var angle = i * angleStep;
			var x = Mathf.Cos(angle);
			var y = CustomShapeFunction(Mathf.Sin(angle), aspectRatio);

			circle[i] = _positions[index] + side * x * _radiusOne + up * y * _radiusOne;
		}
		return circle;
	}
	
	private float CustomShapeFunction(float y, float aspectRatio)
	{
		float smoothFactor = 0.18f; 
		return Mathf.SmoothStep(-aspectRatio, aspectRatio, y * aspectRatio) * smoothFactor;
	}
}